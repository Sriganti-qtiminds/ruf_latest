    ...
    const AuthModal = ({ isOpen, onClose, triggerBy = "/" }) => {
      ...
      useEffect(() => {
        console.log("AuthModal: Render attempt - isOpen:", isOpen, "triggerBy:", triggerBy);
      }, [isOpen, triggerBy]);
      ...
      const handleMobileConfirm = async () => {
        ...
        try {
          const response = await axios.post(`${apiUrl}/auth/checkMobile`, {
      ...
      const handleLogin = async () => {
        ...
        try {
          ...
          console.log("AuthModal: Login response data:", data);
          ...
          if (data.token && data.isMobile) {
            ...
            console.log("AuthModal: Set userData:", { id: data.id, role, userName: data.username });
      ...
      const handleGoogleAuth = async () => {
        ...
        try {
          ...
          const response = await axios.post(`${apiUrl}/auth/g_login`, {
            uid: user.uid,
            email: user.email,
            displayName: name || user.displayName,
          ...
          const data = response.data;
          console.log("AuthModal: Google auth response data:", data);
      ...
      const submitMobileNumber = async () => {
        try {
          const response = await axios.put(`${apiUrl}/auth/addMobileNumber`, {
          ...
          if (response.status === 200) {
            ...
            if (afterLoginData.token) {
              ...
              const role = afterLoginData.role ? afterLoginData.role.toLowerCase() : "user";
              await setUserData({
                id: afterLoginData.id,
                role: role,
                userName: afterLoginData.username,
              });
              console.log("AuthModal: Set userData after mobile:", { id: afterLoginData.id, role, userName: afterLoginData.username });
          ...
          displayMessage("error", `Error adding mobile number: ${error.message}`);
        }
      };
      ...
      useEffect(() => {
        if (isOpen) {
          ...
          setIsLogin(true);
          if (triggerBy !== "/") {
            displayMessage("error", "Please log in to access this page.");
          }
          setIsMobileConfirmed(false);
          setAfterLoginIsMobile(false);
          setName("");
