...
class TableInfo1 {
  ...
  async getJsonData1(jsonfilename) {
    try {
  ...
  async getJsonData(jsonfilename, configKey) {
  try {
    this.jsonfilepath = path.join(__dirname, folderPath, jsonfilename);
    console.log("Reading JSON file:", this.jsonfilepath);
    ...
    if (!parsed[configKey]) {
      console.error(`Key '${configKey}' not found in JSON.`);
      return null;
    }

    this.jsonData = { tbl_info: parsed[configKey] }; // 🔥 wrap it to match existing structure
...
class TableInfo {
  ...
  async getJsonData(jsonfilename, configKey) {
    try {
      ...
      if (!parsed[configKey]) return null;
      this.jsonData = { tbl_info: parsed[configKey] };
      return this.jsonData;
    } catch (error) {
  ...
  async setValues() {
    if (!this.jsonData.tbl_info) return;
    const { tbl_info } = this.jsonData;
    this.tblName = await this.getParsedValue(tbl_info.tbl_name);
    this.tblNameAdd = await this.getParsedValue(tbl_info.tbl_nameadd);
    this.redisKey = await this.getParsedValue(tbl_info.tbl_rediskey);
    this.tblFields = await this.getParsedValue(tbl_info.tbl_fields);
    this.tblJoinClause = await this.getParsedValue(tbl_info.tbl_join_clause);
    this.tblWhereClause = await this.getParsedValue(tbl_info.tbl_where_clause);
  ...
  async getRawTableName() {
    if (!this.tblNameAdd) await this.setValues();
    return this.tblNameAdd;
  }
   async getRedisKey() {
    if (!this.redisKey) await this.setValues();
    return this.redisKey;
  ...
  async getParsedValue(value) {
   if (Array.isArray(value)) {
    if (value.length === 0 || typeof value[0] !== "string") return "";
    return value.join(value[0].includes("JOIN") ? "\n" : ", ");
  }
...
async function getTableMetaData(jsonfilename) {
  ...
  return { tableName, tableFields, joinClause };
}
